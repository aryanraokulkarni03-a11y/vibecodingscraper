...
from config import TMP_DIR, TrendReport, get_reports_dir, get_date_str
...
<body>
    <h1>🚀 Vibe-Coding Trend Report</h1>
    ...
    <h2>🚀 Top 10 Trending AI Tools Analysis</h2>
    {trending_tools}

    <hr>
    <p style="text-align: center; color: #888; font-size: 0.8em;">
        Generated by Vibe-Coding Scraper • {year}
    </p>
</body>
</html>
"""

TRENDING_TOOL_TEMPLATE = """
    <div class="opp-card" style="border-left: 4px solid #8b5cf6;">
        <div class="opp-header">
            <h3><a href="{url}" target="_blank">{name}</a></h3>
            <span class="tag" style="background: #e0e7ff; color: #4338ca;">{validation}</span>
        </div>
        <p><strong>What it does:</strong> {what_it_does}</p>
        <div style="background: #fffbeb; padding: 10px; border-radius: 4px; margin: 10px 0;">
...
OPP_TEMPLATE = """
    <div class="opp-card">
        ...
        <h3><a href="{url}" target="_blank">{name}</a></h3>
        <p>{description}</p>
        <p><strong>💡 Why Vibe-Codeable:</strong> {reason}</p>
        <div class="meta">
            <span class="tag">⏱️ {time}</span>
...
def generate_html_report(report: TrendReport, analysis: dict) -> Path:
    ...
    trending_html = ""
    for tool in analysis.get("trending_tools_analysis", []):
        rev_items = "".join([f"<li>{item}</li>" for item in tool.get("revenue_potential", [])])
        trending_html += TRENDING_TOOL_TEMPLATE.format(
            name=tool.get("name"),
            url=tool.get("url", "#"),
            validation=tool.get("validation", "Unverified"),
            what_it_does=tool.get("what_it_does", ""),
            review=tool.get("review", ""),
    ...
    html_content = HTML_TEMPLATE.format(
        ...
        trending_tools=trending_html,
        year=year
    )
    
    output_path = get_reports_dir() / f"report_{get_date_str()}.html"
